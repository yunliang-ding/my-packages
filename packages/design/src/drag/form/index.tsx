  ...
  props.children.forEach((item, index, items) => {
    ...
    item.itemRender = (vDom: ReactNode, option: any) => {
      ...
      const VNode = (
        <Drag
          dom={vDom}
          label={`${item.type}-${itemProps.key}`}
          virtual={item.virtual}
          mask={!isWrap(item)}
          selected={itemProps.key === (window as any).__itemSelectKey__}
          onDelete={() => {
      ...
      <ComponentWrap
        ...
        schema={items.map((item: any, currentIndex: number) => {
          ...
          return {
            ...
            itemRender(vDom: ReactNode, { itemProps }) {
              const VNode = (
                <Drag
                  dom={vDom}
                  virtual={item.virtual}
