            ...
            if (target) {
              ...
              if (target.isSameNode(container)) {
                const changedNodes = [].slice
                  ...
                  .concat(
                    [].slice.call(record.addedNodes).map((x) => x.parentNode),
                  );

                return changedNodes.some(
                  (x: Node) => x === watermarkDiv.current,
                );
              }
              if (
                target.isSameNode(watermarkDiv.current) &&
